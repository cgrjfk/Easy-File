...
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QPushButton, QFileDialog, QLabel, QLineEdit, \
    QMessageBox, QHBoxLayout, QTextEdit, QProgressBar, QGridLayout, QDesktopWidget
...
class PDFToWordGUI(QWidget):
    ...
    def initUI(self):
        ...
        self.setWindowIcon(QIcon('process_icon/pdf.png'))  # 设置窗口图标
        ...
        self.setWindowTitle('PDF 转 Word 转换器')
        ...
        '''# 按钮连接槽函数
        browse_input_file_button = QPushButton('浏览输入文件', self)
        browse_input_file_button.clicked.connect(self.browse_input_file)
        layout.addWidget(browse_input_file_button)

        browse_input_folder_button = QPushButton('浏览输入文件夹', self)
        browse_input_folder_button.clicked.connect(self.browse_input_folder)
        layout.addWidget(browse_input_folder_button)

        browse_output_button = QPushButton('浏览输出路径', self)
        ...
        convert_button = QPushButton(QIcon("process_icon/convert.png"), 'pdf->docx文件', self)
        convert_button.clicked.connect(self.convert)
    ...
    def browse_input_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, "选择 PDF 文件", "", "PDF 文件 (*.pdf);;所有文件 (*)",
                                                   options=options)
        if file_path:
            self.input_path_edit.setText(file_path)
    ...
    def convert(self):
        ...
        converter = FileConverterFactory.create_converter('pdf_to_word', os.cpu_count(),
    ...
    # 显示结果
    def display_result(self, path_list):
